...
export const getBS = async ([by, bm, bd]: number[]) => {
  ...
  $(".trrestitle").children(":nth-child(1)").not(":first").each((i, elem) => {
    stars[i] = {
      name: "",
      constellation: "",
      dist: "",
  ...
  $(".trrestitle").children(":nth-child(2)").not(":first").each((i, elem) => {
    stars[i].dist = $(elem).text().trim();
  });
  $(".trrestitle").children(":nth-child(3)").not(":first").each((i, elem) => {
    stars[i].offset = $(elem).text().trim();
  });
  $(".trrestitle").children(":nth-child(4)").not(":first").each((i, elem) => {
    stars[i].mag = $(elem).text().trim();
  });
  $(".trrestitle").children(":nth-child(5)").not(":first").each((i, elem) => {
    stars[i].constellation = $(elem).text().trim();
    $(elem).children().each((j, elem) => {
      stars[i].href[j] = $(elem).attr("href")?.trim()!;
    });
  });
  $(".trrestitle").children(":nth-child(7)").not(":first").each((i, elem) => {
    stars[i].hip = $(elem).text().trim();
  });
  $(".trrestitle").children(":nth-child(8)").not(":first").each((i, elem) => {
    stars[i].hd = $(elem).text().trim();
  });
  $(".trrestitle").children(":nth-child(9)").not(":first").each((i, elem) => {
    stars[i].hrc = $(elem).text().trim();
  });
  $(".trrestitle").children(":nth-child(10)").not(":first").each((i, elem) => {
